...
def place_header_text (header_text, header_position):
    header_note = DB.TextNote.Create(revit.doc, view.Id, header_position, header_text, get_any_text_type_id())
    return header_note
...
with revit.Transaction("do smth"):
    for header in colour_scheme_od:
        ...
        place_header_text(header, header_position)
        for component in colour_scheme_od[header]:
            ...
            text_note = DB.TextNote.Create(revit.doc, view.Id, label_position, label_txt, get_any_text_type_id())
